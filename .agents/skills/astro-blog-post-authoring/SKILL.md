...
description: Create, edit, or structurally revise Markdown posts for this Astro blog. Use for `src/content/posts/*/index.md`, new post folders, frontmatter, tags, slugs, links, footnotes, code, math, embeds, drafts, and publication-ready content checks; pair with the language-specific writing-style skill when prose voice matters.
---
...
1. Read `src/content.config.ts` for the current schema.
...
- Create `src/content/posts/<slug>/index.md` and keep post-specific assets in that folder.
...
- Treat `src/content.config.ts` as authoritative if this example and the schema diverge.
...
- Preserve syntax supported by `astro.config.mjs`: GFM, math, footnotes, code metadata, and inline HTML.
