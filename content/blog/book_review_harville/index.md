...
正方行列$\textbf{A}$のある固有値$\lambda$に対して、$\mathrm{dim}[\mathcal{N}(\textbf{A}-\lambda\textbf{I})]$を$\lambda$の**幾何学的重複度**と呼びます。また、$\lambda$の**特性方程式**の解としての重複度を$\lambda$の**代数的重複度**と呼びます。これら2種類の重複度は一般に一致せず、任意の固有値に対して（幾何的重複度）$\leq$（代数的重複度）という関係が成り立ちます。行列$\textbf{A}$は幾何学的重複度を含めた固有値の数だけ互いに線形独立な固有ベクトルを持ちます。  
