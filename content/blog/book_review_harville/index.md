...
以下の性質は計算上重要です。
- 2つの列または行を交換すると、行列式の符号が反転する
- ある列または行を定数倍すると、行列式も定数倍される
- 列（行）に他の列（行）の定数倍を加えても行列式は変化しない
...
本章では、行列の二次形式や正定値性について議論します。12の節からなる長い章なので、内容をかなり絞って書きます。  
$n \times n$行列$\textbf{A}$が任意の$\textbf{x}\in \mathbb{R}^n$に対して$\textbf{x}'\textbf{Ax} \geq0$を満たすとき、$\textbf{A}$が**非負定値 (nonnegative definite)** であると言います。特に、$\textbf{x}'\textbf{Ax} >0$を満たすとき、$\textbf{A}$が**正定値 (positive definite)** であると言います。非負定値であるが正定値でないとき、$\textbf{A}$が**半正定値 (positive semidefinite)** であると言います。ここまでに登場した様々な行列をベン図にまとめました。なお、この本では複素数からなる行列は扱いません。
